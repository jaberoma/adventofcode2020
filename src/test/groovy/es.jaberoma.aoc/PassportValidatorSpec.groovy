...
import es.jaberoma.aoc.day4.PassportReader
import es.jaberoma.aoc.day4.PassportValidator
...
class PassportValidatorSpec extends Specification {
    ...
    @Unroll
    void 'Passport line validator, validates all required fields, according to validator rules'() {
        when:
        boolean isValid = PassportValidator.validate(passport)
    ...
    void 'Passport reader generates list of passports'() {
        when:
        List<String> passports = PassportReader.readPassports('day4_sample.input')
    ...
    void 'Sample passports file contains 2 valid passports'() {
        given:
        List<String> passports = PassportReader.readPassports('day4_sample.input')

        when:
        long numValid = PassportValidator.validate(passports)

        then:
        numValid == 2
    }

    void 'Day 4 passports file contains valid passports'() {
        given:
        List<String> passports = PassportReader.readPassports('day4.input')
